...
def main() -> None:
    ...
    for t in UNIVERSE:
        try:
            ...
            r = run_backtest(df, EliasEngine(willing_to_add=True, index_close=index_close))
    ...
    cd.to_csv(OUT / "cd_validation.csv")
    ...
    cmp["cd_minus_nocd"] = cmp["elias_CD"] - cmp["elias_noCD"]
    cmp["cd_minus_naive"] = cmp["elias_CD"] - cmp["naive"]
    cmp = cmp.sort_values("cd_minus_nocd", ascending=False)
    cmp.to_csv(OUT / "cd_vs_baseline.csv")

    print("\n=== CD-gated vs baseline (total_return) ===")
    print(cmp.round(3).to_string())
    print("\nMEDIANS:")
    print(cmp.median().round(3).to_string())
    improved = int((cmp["cd_minus_nocd"] > 0.01).sum())
    worse = int((cmp["cd_minus_nocd"] < -0.01).sum())
